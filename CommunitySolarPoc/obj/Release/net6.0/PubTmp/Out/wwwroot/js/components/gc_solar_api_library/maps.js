...
function getLatLong() {
  ...
  const formattedAddress = address.split(" ").join("+");
    //const lat = 39.82294065;
    //const lon = -86.14409793;
  // Form the request URL
    const url = `https://maps.googleapis.com/maps/api/geocode/json?address=${formattedAddress}&key=${apiKey}`;
    //const url1 = `https://solar.googleapis.com/v1/buildingInsights:findClosest?location.latitude=${lat}&location.longitude=${lat}&requiredQuality=HIGH&key=${apiKey}`
    ;
